...
export class SplitTicketingEngine {
  ...
  public static calculateEconomics(request: PricingCalculationRequest): SplitTicketComparison {
    ...
    // 2. Расчет стоимости стандартного прямого/сквозного маршрута или пользовательской цены
    let standardTotalPrice = 0;
    const hasUserTarget = Boolean(request.userTargetPrice && request.userTargetPrice > 0);

    if (hasUserTarget) {
      standardTotalPrice = request.userTargetPrice!;
    } else if (standardItinerary) {
    ...
    const benchmarkLabel = hasUserTarget
      ? `Ваша цена на ${request.userTargetSource || 'стороннем сайте'}`
