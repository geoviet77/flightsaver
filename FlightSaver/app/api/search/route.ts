...
- Если все параметры согласованы (есть откуда, куда, дата вылета, понятен тип поездки) — установи is_complete = true и выдай: "Подобрал для вас лучшие варианты перелета:".
 ...
 */
async function fetchOrBridgeFlights(state: any): Promise<Flight[]> {
  ...
  if (token) {
    try {
      rawDuffelOffers = await queryDuffelDirect(origin, destination, state, token);
    } catch {
      rawDuffelOffers = [];
    }
  }
  ...
  if (hubConnection) {
    return buildRealisticSplitBridge(state, hubConnection);
  }

  // 3. Для остальных международных пар строим валидный сплит через мировой хаб (IST/DXB)
  return buildInternationalSplitFlight(state);
}

async function queryDuffelDirect(origin: string, destination: string, state: any, token: string): Promise<Flight[]> {
  ...
  return offers.slice(0, 4).map((offer: any, idx: number): Flight => {
    ...
    const benchmarkPrice = calculateRealisticBenchmark(origin, destination, state.passengers_count || 1);
    const marketPrice = Math.max(benchmarkPrice, Math.round(totalRub * 1.25));
    const savedAmount = Math.max(0, marketPrice - totalRub);
    ...
    return {
      ...
      pricing: {
        ...
        marketPrice,
        savedAmount,
        savedPercentage: Math.round((savedAmount / marketPrice) * 100),
 ...
 */
function buildRealisticSplitBridge(state: any, hub: HubConnection): Flight[] {
  ...
  const totalPrice = (dLeg.priceRub + intlPriceRub) * passengers;
  const benchmarkPrice = calculateRealisticBenchmark(originIata, destIata, passengers);
  const marketPrice = Math.max(benchmarkPrice, Math.round(totalPrice * 1.28));
  const savedAmount = marketPrice - totalPrice;
  const savedPercentage = Math.max(15, Math.round((savedAmount / marketPrice) * 100));
  ...
  return [
    {
      ...
      pricing: {
        ...
        savedAmount,
        savedPercentage,
        netSupplierFare: Math.round(totalPrice * 0.94),
        serviceFee: Math.round(totalPrice * 0.06),
        segmentBreakdowns: [
          { segmentTitle: `Сегмент 1: ${originIata} → ${hub.hubIata}`, providerName: dLeg.airline, price: dLeg.priceRub * passengers, currency: 'RUB' },
          { segmentTitle: `Сегмент 2: ${hub.hubIata} → ${destIata}`, providerName: `${intlAirline} NDC`, price: intlPriceRub * passengers, currency: 'RUB' },
        ],
        splitSavingsReason: `Комбинированный сплит: Сегмент 1 (${dLeg.airline}) + Сегмент 2 (${intlAirline} NDC) через ${hubMeta.city}`,
      },
      isBestValue: true,
      isFastest: true,
...
function buildInternationalSplitFlight(state: any): Flight[] {
  ...
  const totalPrice = 36800 * passengers;
  const marketPrice = Math.round(totalPrice * 1.32);
  const savedAmount = marketPrice - totalPrice;
  ...
  return [
    {
      ...
      pricing: {
        ...
        savedAmount,
        savedPercentage: 24,
        netSupplierFare: Math.round(totalPrice * 0.95),
        serviceFee: Math.round(totalPrice * 0.05),
        segmentBreakdowns: [
          { segmentTitle: `${originIata} → IST`, providerName: 'Turkish Airlines', price: Math.round(totalPrice * 0.48), currency: 'RUB' },
          { segmentTitle: `IST → ${destIata}`, providerName: 'Turkish Airlines', price: Math.round(totalPrice * 0.52), currency: 'RUB' },
        ],
        splitSavingsReason: 'Сплит-тариф со стыковкой в Стамбуле и отелем STPC',
      },
      isBestValue: true,
      isFastest: false,
      isStpcEligible: true,
      ...
      baggageDescription: 'Багаж 23 кг + ручная кладь 8 кг',
      cabinClass: 'Economy',
      tags: ['🎁 Отель STPC 4★', 'Duffel Verified'],
    },
  ];
}
...
function extractDeterministicState(text: string, context: any) {
  ...
  return {
    ...
    quick_options: hasAll
      ? ['🔄 Добавить обратный билет', '👥 2 пассажира', '💎 Бизнес-класс']
      : ['Иркутск → Бангкок', 'Красноярск → Мюнхен'],
  };
}
